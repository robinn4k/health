...
export default function RecipeSearch({ mealType, onClose }) {
  ...
  return (
    <div className="recipe-overlay safe-top">
      {/* Header */}
      <div className="px-5 pt-4 pb-3 flex items-center justify-between">
        <div>
          <p className="font-mono text-[10px] font-semibold tracking-[3px] uppercase" style={{ color: 'var(--gold)' }}>
            Recetas
          </p>
      ...
      <div className="px-5 pb-3 relative">
        <span className="absolute left-8 top-1/2 -translate-y-1/2 text-sm" style={{ color: 'var(--text3)' }}>
          🔍
        </span>
        <input
          type="text"
      ...
      <div className="px-5 pb-4 flex flex-wrap gap-1.5">
        {quickTags.map(tag => (
          <button
            ...
            className="font-mono text-[10px] font-medium px-2.5 py-1.5 rounded-full border transition-all duration-200"
            style={{
      ...
      {/* Results */}
      <div className="px-3 pb-24 flex flex-col gap-2">
        {filtered.length === 0 && (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className="flex items-center gap-1 font-mono text-[10px]">
                      <span className="w-1 h-1 rounded-full" style={{ background: 'var(--carb)' }} />
                      <span style={{ color: 'var(--text3)' }}>{recipe.macros.c}C</span>
                    </span>
                    <span className="flex items-center gap-1 font-mono text-[10px]">
                      <span className="w-1 h-1 rounded-full" style={{ background: 'var(--prot)' }} />
                      <span style={{ color: 'var(--text3)' }}>{recipe.macros.p}P</span>
                    </span>
                    <span className="flex items-center gap-1 font-mono text-[10px]">
                      <span className="w-1 h-1 rounded-full" style={{ background: 'var(--fat)' }} />
                ...
                <div className="px-4 pb-4">
                  ...
                  <div className="flex flex-col gap-1.5">
                    {recipe.ingredients.map((ing, i) => (
                      <div key={i} className="flex items-start gap-2.5 px-2.5 py-2 rounded-lg" style={{ background: 'rgba(255,255,255,0.015)' }}>
                        <div className="w-1.5 h-1.5 rounded-full mt-1.5 shrink-0" style={{ background: typeColors[ing.type] || 'var(--text3)' }} />
                        <div className="flex-1 flex items-baseline justify-between gap-2">
                          <p className="text-[13.5px] font-medium leading-snug">{ing.name}</p>
                          <p className="font-mono text-[10.5px] shrink-0" style={{ color: 'var(--text3)' }}>{ing.qty}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  >
                    <span className="flex items-center gap-1.5 font-mono text-[11px] font-medium">
                      <span className="w-1.5 h-1.5 rounded-sm" style={{ background: 'var(--carb)' }} /> {recipe.macros.c}C
                    </span>
                    <span className="flex items-center gap-1.5 font-mono text-[11px] font-medium">
                      <span className="w-1.5 h-1.5 rounded-sm" style={{ background: 'var(--prot)' }} /> {recipe.macros.p}P
                    </span>
                    <span className="flex items-center gap-1.5 font-mono text-[11px] font-medium">
                      <span className="w-1.5 h-1.5 rounded-sm" style={{ background: 'var(--fat)' }} /> {recipe.macros.g}G
                    </span>
                    <span className="ml-auto font-mono text-[11px] font-bold" style={{ color: 'var(--gold)' }}>
                      {recipe.kcal} kcal
                    </span>
                  </div>
                </div>
        ...
        {/* Count */}
        {filtered.length > 0 && (
          <p className="text-center font-mono text-[10px] py-3" style={{ color: 'var(--text3)' }}>
